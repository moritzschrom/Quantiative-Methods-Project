...
Die Forschungsfrage lautet daher **Gibt es einen Zusammenhang zwischen dem Sternzeichen einer Person und ihren Charaktereigenschaften?** 
...
* Intuition (0) ... Sensorik (1)
* Denken (0) ... Fühlen (1)
* Wahrnehmen (0) ... Beurteilen (1)
...
* Fische (19.02. - 20.03.)
Die Daten wurden in einer geteilten Datenbank gesammelt, liegen in einer exportierten CSV-Datei vor und enthält TODO Datensätze.
Bei der Analyse der Daten  ist erkennbar, dass die Sternzeichen in der Stichprobe nicht gleich oft vorkommen. 
TODO: So kommt das Sternzeichen ... mit einer Population von ... am häufigsten vor, das Sternzeichen ... mit einer Population von ... am seltensten (auf die Stichprobe bezogen).
Es ist zu erkennen, dass Charaktereigenschaften in der Stichprobe bei unterschiedlichen Sternzeichen unterschiedlich häufig auftreten. TODO: So gibt es in der Stichprobe kaum Personen mit Sternzeichen Löwe und Charaktereigenschaft Introversion, jedoch viele Personen mit Sternzeichen Stier und Charaktereigenschaft Introversion

TODO: Conclusion von Einleitung
...
* Charaktereigenschaft1: Extraversion (0) ... Introversion (1)
* Charaktereigenschaft2: Intuition (0) ... Sensorik (1)
* Charaktereigenschaft3: Denken (0) ... Fühlen (1)
* Charaktereigenschaft4: Wahrnehmen (0) ... Beurteilen (1)
...
* Mehr als zwei kategoriale Variablen:
Wir untersuchen zunächst den Einfluss des kategorialen Merkmals Tierkreiszeichen auf die kategorialen Merkmale der Charaktereigenschaften. Hierbei wird jede Charaktereigenschaft sowohl einzeln als auch alle Charaktereigenschaften gesammelt analysiert.
...
Zur Untersuchung gelangt eine Stichprobe mit TODO Beobachtungen und fünf Merkmalen:
...
Es ist erkennbar, dass die Sternzeichen in der Stichprobe nicht gleich oft vorkommen. 
#TODO: So kommt das Sternzeichen ... mit einer Population von ... am häufigsten vor, das Sternzeichen ... mit einer Population von ... am seltensten (auf die Stichprobe bezogen).
...
Manche der Charaktereigenschaften (Intuition/Sensorik, Denken/Fühlen, Beurteilen/Wahrnehmen) kommen in der Stichprobe in etwa gleich häufig vor. Einzig die Charaktereigenschaft Extraversion/Introversion scheint etwas ungleich verteilt.
...
Zunächst kombinieren wir die einzelnen Kontingenztabellen zu einer gemeinsamen Tabelle, welche die Sternzeichen und die Summe aller Charaktereigenschaften (jeweils 4 binäre kategoriale Variablen) enthält:
...
Der P-Wert liegt mit TODO in beiden Fällen weit über dem Signifikanzniveau von 0.05, die Homogenitätshypothese wird also beibehalten. Die vorliegende Stichprobe spricht gegen signifikanten Unterschiede in den beobachteten Charaktereigenschaften zwischen vergleichbaren Gruppen von Menschen mit demselben Sternzeichen. Es konnten kein signifikant höheres oder niederes Auftreten bestimmter Charaktereigenschaften bei unterschiedlichen Sternzeichen nachgewiesen werden.


# Fazit

* Zusammenfassung
  * Die in diesem Bericht dargelegte Fragestellung "Gibt es einen Zusammenhang zwischen dem Sternzeichen einer Person und ihren Charaktereigenschaften?" wurde anhand fünf Merkmalen, die pro Respondent:in erhoben wurden, untersucht. Das *Homogenitätsproblem* wurde anhand der statistischen Methoden der Kovarianzanalyse und an den mehr als zwei kategorialen Variablen überprüft.
  * Die Anzahl der jeweiligen erhobenen Sternzeichen ist in der Stichprobe nicht gleichverteilt, die Paare der Charaktereigenschaften (Intuition/Sensorik, Denken/Fühlen, Beurteilen/Wahrnehmen) kommen in etwa gleich häufig vor. Die Ausnahme stellen hier die Eigenschaften Extraversion/Introversion dar, diese scheinen ungleich verteilt zu sein.
  * Nach der umfassenden Durchführung des Chi-squared-Tests nach Pearson konnte festgestellt werden, dass die Homogenitätshypothese nicht verworfen werden kann, da der P-Wert weit über dem verwendeten Signifikanzniveau von 0.05 liegt. Mit den vorliegenden Daten kann keine Signifikanz im Auftreten von Charaktereigenschaften bei Sternzeichen festgestellt werden.
